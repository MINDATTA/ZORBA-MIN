...
CREATE FUNCTION DBO.OM_ITEM_ID (@ITEMID varchar(50)) 
RETURNS varchar(50) 
AS 
BEGIN 
DECLARE @CHECK_VALID_ITEM_ID varchar(50)

SET @CHECK_VALID_ITEM_ID = CASE
					WHEN ((LEN(@ITEMID)=5 AND @ITEMID LIKE 'I%' AND(@ITEMID LIKE '__P%' OR @ITEMID LIKE'__M%'))) THEN @ITEMID 
					ELSE NULL
					END

RETURN @CHECK_VALID_ITEM_ID
END

SELECT DBO.OM_ITEM_ID ('IKP36')
SELECT DBO.OM_ITEM_ID ('IKM40')
SELECT DBO.OM_ITEM_ID ('IKP33')
SELECT DBO.OM_ITEM_ID ('IKB33')
SELECT DBO.OM_ITEM_ID ('IKM3')

------------------------------------------------------------------

ALTER FUNCTION DBO.OM_ITEM_ID (@ITEMID varchar(50)) 
RETURNS varchar(50) 
AS 
BEGIN 
DECLARE @CHECK_VALID_ITEM_ID varchar(50)
...
SET @CHECK_VALID_ITEM_ID = CASE
					WHEN LEN(@ITEMID)=5 AND @ITEMID LIKE 'I%' AND(@ITEMID LIKE '__P%' OR @ITEMID LIKE'__M%') AND RIGHT(@ITEMID, 1) LIKE '[4-9]' THEN @ITEMID 
					ELSE NULL
					END
...
ELSE NULL
END  AS ITEMID, ITEM_NAME,PRICE,QTY,SOLD_DATE
FROM R_OM_STOCK1 WHERE ITEMID=ITEMID
