...
BEGIN
	...
	IF ((SELECT COUNT(*) FROM OM_BANK_ACCOUNT WHERE ACCT_NO = @ACCT_NO) > 0)  -- THIS WILL CHECK IF THAT ACCT_NO IS THERE OR AS IF WE ARE USING PRIMARY KEY AND FK
		...
		ELSE
		BEGIN
			IF ( (SELECT BALANCE FROM OM_BANK_ACCOUNT WHERE ACCT_NO = @ACCT_NO) >= @AMOUNT) 
				BEGIN
					INSERT INTO OM_TRANSACTION VALUES (@ACCT_NO, @ACTIVITY,@AMOUNT, GETDATE())
...
EXEC OM_SP_INSERT_OM_TRANSACTION 'BOA_5001','DEPOSIT',1000 -- ACCT_NO NOT EXIST
SELECT * FROM OM_BANK_ACCOUNT
SELECT * FROM OM_TRANSACTION


EXEC OM_SP_INSERT_OM_TRANSACTION 'BOA_4561','WITHDRAW',5000 -- ACCT_NO NOT EXIST
SELECT * FROM OM_BANK_ACCOUNT
SELECT * FROM OM_TRANSACTION


EXEC OM_SP_INSERT_OM_TRANSACTION 'BOA_4564','WITHDRAW',800
SELECT * FROM OM_BANK_ACCOUNT
SELECT * FROM OM_TRANSACTION



EXEC OM_SP_INSERT_OM_TRANSACTION 'BOA_4564','DEPOSIT',00
SELECT * FROM OM_BANK_ACCOUNT
SELECT * FROM OM_TRANSACTION
